...
# # 泛癌模型--训练组
# write_res('data/Training_RF_Prob.txt', 'data/Training_RF_Prob_Pan_Predicted.txt', '../r/data/Pan_data/Thresholds.txt', False)
# # 癌症特异模型--训练组
...
for i in range(3):
    rf = open('data/Training_RF_Prob_Pan_Predicted.txt', 'r')
    line = rf.readline()
    tp = tn = fp = fn = 0
    while line != '':
        line = line.strip().split('\t')
