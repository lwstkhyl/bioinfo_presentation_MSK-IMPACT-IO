...
library(ggpubr);
# 数据处理
data <- read.table('../pycharm/data/Test_RF_Prob.txt', header = T, sep = '\t');  # 验证组
cancer_type <- c("Pan-cancer", "Melanoma", "NSCLC","Others");
data1_p <- c();  # 两组数据的p值
data2_p <- c();
# 泛癌
data_pan <- data;  # 先复制一遍所有的数据，作为泛癌组，之后再添加各种癌症单独的样本
